...
export class Ng2Uploader {
  ...
  xhrCollection: XMLHttpRequest[] = [];
  fileUploadingCollection: UploadedFile[] = [];

  uploadStop(){
      this.xhrCollection.forEach((item:XMLHttpRequest) => {
          item.abort();
      });
      this._queue = [];
      this.xhrCollection = [];
      this.fileUploadingCollection = [];
      this.formCollection = [];
  ...
  uploadFilesInQueue(): void {
    let newFiles = this._queue.filter((f:any) => { return !f.uploading; });
    newFiles.forEach((f) => {
      this.uploadFile(f);
    });
  };
  ...
  uploadFile(file: any): void {
    ...
    this.fileUploadingCollection.push(uploadingFile);
